...
if __name__ == "__main__":
    ...
    server_db_path=""   ###本機 DataDB 已經有 srj 檔，留空字串跳過雲端下載(避免重複下載)
    ...
    ##-------step 1. DataDB 本機已有 srj 檔，但 Model/Dataset 目前是空的(data_parsing 還沒真正成功寫出資料)，
    ##                所以要跑完整的 data_processing(跳過雲端下載，但仍需要 data_parsing + data_arrangement)-------
    for user_info in user_information:
        uuid = user_info[0]
        srj_db_path=str(mother_path/"DataDB"/uuid)   ###srj檔放置路徑(本機已存在的srj檔都放這裡)
        print('uuid:',uuid)
        errorcode, message = DataArrangement_Obj.data_processing(uuid, srj_db_path, glucosedata_path, basepath, server_db_path, processnum, splitting_ratio)
