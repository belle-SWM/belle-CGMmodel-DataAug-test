...
> 註：這組開關當初加在檔案開頭（模組層級），後來被移到 `__main__` 裡跟 `model_subdir`／`augment` 放在一起（[L2053-2066](M5_DataAug.py#L2053-L2066)）。詳見 2026/09/11 `compare_results.py` 那一節的說明。
...
開關原本加在檔案開頭（模組層級），後來被移進 `if __name__ == "__main__":`（[L2053 起](M5_DataAug.py#L2053)），跟 `model_subdir`／`augment` 放在一起——就設定集中度來說更好找。
...
開關（沿用 `__main__` 那組）：`AUG_SAME_CLASS_MIXUP = True`（[L2058](M5_DataAug.py#L2058)）、`MIXUP_LAMBDA_MIN = 0.5`（[L2066](M5_DataAug.py#L2066)）。
...
# 2026/09/11 WithAug_4 結果判讀，並設定 WithAug_5 = trio + mixup

## 先量雜訊，再看效果
WithAug_1 和 WithAug_2 其實是近乎重複的設定（都是 trio = noise+amp+shift，唯一差別是中間把 `torch.roll` 換成邊緣補值，而那個改動實測影響極小）。它們的差距就是 **run-to-run 雜訊**的估計：
...
**任何小於約 1 個 F1 的三分類差距、小於約 2 個的二分類差距，都不該當真。**

## 三分類：兩個關鍵發現

| 模式 | 平均 ΔF1 | 剔除 2210 | 勝率 | 逐 uuid |
|---|---|---|---|---|
| trio（WithAug_1）| +8.32 | **+3.63** | 5/5 | +1.5, **+27.1**, +8.6, +3.5, +0.9 |
| trio（WithAug_2）| +8.01 | +3.47 | 5/5 | +1.4, +26.2, +8.4, +2.5, +1.6 |
| 純頻域（WithAug_3）| −0.34 | **−0.01** | 2/5 | −5.1, −1.6, +3.3, −0.9, +2.7 |
| 純 mixup（WithAug_4）| +2.54 | **+3.07** | 5/5 | +0.2, +0.4, +4.4, +3.7, +4.0 |

**1. 頻域擾動在三分類上是真的零效果** —— 整體 −0.34、剔除 2210 後 −0.01、勝率只有 2/5。

**2. trio 的 +8.3 幾乎全靠 2210 一個受試者（+27.1）撐起來。剔除它之後 trio（+3.63）和 mixup（+3.07）基本打平。** 2210 剛好是資料最不平衡的（Train High/Normal/Low = 246/2082/418）。而且 mixup 的效果更均勻（+0.2, +0.4, +4.4, +3.7, +4.0，對照雜訊 0.59）。

二分類則相反：頻域是單項最好（+5.50、4/5），但那邊雜訊 1.74，證據力弱得多。

## 離線效率指標在這裡判斷錯了 —— 重要教訓
先前那套「多樣性 ÷ 標籤損失」給頻域 **9.6**、給高斯雜訊 **0.5**，但實際結果完全相反：頻域在三分類零效果，含雜訊的 trio 最強。

先前就寫過「效率排序只能篩掉有害的方法，不能保證有效」，現在有了明確反證。**往後不要用那個指標預測效果，只用它排除明顯會抹掉標籤的方法。**

## 這次的設定：WithAug_5 = trio + mixup
沒有選「mixup + 頻域」，因為頻域在三分類上零效果，加進去主要只是讓每個樣本被擾動更多。選 trio + mixup 的三個理由：

1. 三分類上兩者都是 **5/5 全勝**，頻域不是。
2. **強項在不同受試者上**：trio 幾乎獨吞 2210 的增益，mixup 在其餘四個較平均，有機會相加。
3. **機制互補且剛好互相抵銷缺點**：trio 是逐樣本獨立擾動，mixup 是跨樣本平均；mixup 會把類別內變異壓到 80%（訓練資料比測試乾淨），trio 的高斯雜訊正好把變異加回去。

開關設定見 [L2053 起](M5_DataAug.py#L2053)，`model_subdir="WithAug_5"`。

順帶一提：`conv1` 後面緊接 `BatchNorm1d`，所以 trio 裡的振幅縮放有相當部分會被 BatchNorm 吸收（不是完全，因為 BN 用 batch 統計量，逐樣本的縮放差異會部分留存）。真要拆解 trio 的話，有效成分推測主要是雜訊和平移 —— 尚未驗證。

## 四種疊加的驗證
四個增強同時啟用是先前沒測過的路徑，用 2210 的 train 實測 800 筆：

- 形狀/dtype/NaN/標籤異常：**0 筆**
- 被增強的比例 95%（四種各 p=0.5，至少一種觸發的理論值 93.75%）
- DataLoader 正常，batch 全為有限值

**R-T 間期的保存程度**（這才是帶標籤的量；絕對 T 峰位置對剛性平移不公平，因為平移讓 R 和 T 一起移動）：

| 設定 | 間期不變 | 位移 ≤1 idx | 位移 >5 idx |
|---|---|---|---|
| 全部關閉（對照）| 100.0% | 100.0% | 0.0% |
| 只開 mixup | 73.6% | 90.6% | 0.6% |
| 只開 trio | 62.0% | 79.8% | 0.4% |
| **四種全開** | 49.9% | 73.6% | **1.1%** |

疊加確實會讓 R-T 間期的擾動累積，但尾巴（>5 idx = >20ms，相對於類別間 8–32ms 的差距）只有 1.1%，多數落在 ±1 idx（±4ms，已是 250Hz 的量化下限）。

**要留意的風險**：如果 WithAug_5 的結果比 trio 和 mixup **兩者都差**，最可能的原因就是增強過度。屆時可先把 `MIXUP_LAMBDA_MIN` 調高（例如 0.7，減少混合強度）再試。
